...
APK_PATH="$HOME/storage/downloads/ducksms.apk"
if [ -f "$APK_PATH" ]; then
    echo -e "${GREEN}APK already exists at $APK_PATH, skipping download.${NC}"
else
    echo -e "${CYAN}Downloading the latest APK...${NC}"
    curl -L "https://github.com/mahmudur-rahman-dev/termux-scripts/releases/download/v1.0.0-beta/app-debug.apk" -o "$APK_PATH" || {
